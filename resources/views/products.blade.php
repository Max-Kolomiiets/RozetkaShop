    ...
    @if ($view_info->query_type != 'none')
        <div class="row" id="main-content-filtring">
            ...
            <div class="col filtres accordion" id="filters-section">
                <form
                    @if ($view_info->query_type == 'search') action="{{ route('products.search.filtering') }}"

                @else

                action="{{ route('category.filtering', $view_info->{'url part'}) }}" @endif
                    method="POST">
                    @csrf
                    ...
                    <div class="accordion-item form-group" id="aPrice">
                        <h2 class="accordion-header" id="hPrice">
                            <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#cPrice"
                                aria-expanded="true" aria-controls="cPrice">

                                <h5>Ціна</h5>

                            </button>
                        </h2>
                        <div id="cPrice" class="accordion-collapse collapse show" aria-labelledby="hPrice"
                            ...
                            <div class="accordion-body" id="priceSection">
                                ...
                                <input type="number" name="price-min" onchange="this.form.submit()"
                                    value="{{ $view_info->prices->value_min }}">
                                <label for="price-max">-</label>
                                <input type="number" name="price-max" onchange="this.form.submit()"
                                    value="{{ $view_info->prices->value_max }}">
                    ...
                    <div class="accordion-item form-group" id="aVendor">
                        <h2 class="accordion-header" id="hVendor">
                            <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                data-bs-target="#cVendor" aria-expanded="true" aria-controls="cVendor">
                                <h5>Виробник</h5>
                            </button>
                        </h2>
                        <div id="cVendor" class="accordion-collapse collapse show" aria-labelledby="hVendor"
                            data-bs-parent="#accordionAttributes">
                                ...
                                @foreach ($view_info->vendors as $vendor)
                                    <div class="custom-control custom-checkbox form-group">
                                        <input @if ($vendor->state) checked @endif type="checkbox"
                                            ...
                                            id="vendor-{{ $loop->index }}" name="vendor-{{ $loop->index }}"
                                            value={{ $vendor->alias }}>
                                        <label class="custom-control-label"
                                            for="vendor-{{ $loop->index }}">{{ $vendor->name }}</label>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    @foreach ($view_info->characteristics as $key => $values)
                        ...
                        <div class="accordion-item" id="a{{ $key }}">
                            ...
                            <div id="collapse{{ $key }}" class="accordion-collapse collapse show"
                                aria-labelledby="heading{{ $key }}" data-bs-parent="#accordionAttributes">
                                <div class="accordion-body">
                        ...
                        <div class="product product-filltres">
                            <div
                                ...
                                <a href="{{ route('product.show', $product->id) }}"
                                    ...
                                    <img class="bi" width="100em" height="100em" src="{{ $imagePath }}"
                                        alt="">
                                </a>
                            </div>
                            <div>
    ...
    <script>
        ...
        let pageScript = function() {
            ...
            $('.accordion-item').click(function() {
                ...
                sessionStorage[`accordionState${$(this).attr('id')}`] =
                    `${aria_expanded};${div_class};${button_class}`;
            });
